    ...
    <Cliente>
        ...
        <v-container class="justify-space-around">
            <v-row>
                <v-col cols="12" md="12" class="pa-0  mt-15 deep-orange" >
                    <v-card-actions>
                        ...
                        <v-btn icon color="#FF5E00" outlined rounded class="white" title="Emitir Relatórios"
                    ...
                    <v-hover v-slot="{ hover }">
                        ...
                        <v-card :loading="!loading" class=" elevation-10 pa-2 ma-3 border" :elevation="hover ? 10 : 0">
                            ...
                            <v-card-actions class="justify-end">
                                ...
                                <v-btn v-if="user.tipo_user_id == 1 && item.estado_imoveis_id == 8" icon color="green"
                                    outlined rounded title="Confirmação do arrendamento da casa" @click="confirmarArrendar(item.id)">
                                    <v-icon>
                                        fa fa-check
                                    </v-icon>
                                </v-btn>
                                <v-btn v-if="user.tipo_user_id == 1 && item.estado_imoveis_id == 8" icon color="red"
                                    outlined rounded title="Confirmação do arrendamento da casa" @click="canceladoArrendar(item.id)">
                                    <v-icon>
                                        fa fa-times
                                    </v-icon>
                                </v-btn>
            ...
            <div class="text-center">
                ...
                <v-snackbar v-model="snackbarArrendar" :multi-line="multiLine" :timeout="8000" outlined
                    color="deep-purple accent-4">
                    {{ textConfirmarArrendamento }}
                    <v-btn color="indigo" text @click="snackbarArrendar = false">
                        Close
                    </v-btn>
                </v-snackbar>
                <v-snackbar v-model="snackbarArrendarCancelado" :multi-line="multiLine" :timeout="8000" outlined
                    color="deep-purple accent-4">
                    {{ textCanceladoArrendamento }}
                    <v-btn color="indigo" text @click="snackbarArrendarCancelado = false">
                        Close
                    </v-btn>
                </v-snackbar>
            </div>
...
export default {
    ...
    data: () => ({
        ...
        textvalidado: `Confirmaste a continuação da negociação da casa`,
        textConfirmarArrendamento: `A casa foi confirmada!!`,
        textCanceladoArrendamento: `Contrato da casa negado!!`,
    ...
    methods: {
        ...
        confirmarArrendar(item) {
            this.validar_processo.imovel_id = item
            this.$inertia.post("/confirma-arrendar", this.validar_processo,
            )
            this.loading = true;
            this.snackbarArrendar = true;
        },
        canceladoArrendar(item) {
            this.validar_processo.imovel_id = item
            this.$inertia.post("/cancelado-arrendar", this.validar_processo,
            )
            this.loading = true;
            this.snackbarArrendarCancelado = true;
        },
